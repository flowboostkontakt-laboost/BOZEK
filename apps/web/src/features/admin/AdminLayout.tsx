...
export function AdminLayout() {
  ...
  return (
    <div className="flex h-[100dvh] bg-bg text-ink">
      {/* Sidebar stały — tylko desktop */}
      <aside className="hidden w-64 shrink-0 flex-col border-r border-line bg-surface-1 lg:flex">
        <SidebarContent />
      </aside>

      {/* Drawer — tylko mobile, renderowany gdy otwarty */}
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-black/50" onClick={() => setOpen(false)} />
          <aside className="relative flex h-full w-72 max-w-[82%] flex-col border-r border-line bg-surface-1" style={{ paddingTop: "env(safe-area-inset-top)" }}>
